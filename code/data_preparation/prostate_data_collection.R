...
# ffun <- filterfun(pOverA(p = 0.2, A = 50))
...
# table(filt7)                       ## True = 39 genes only
# expr7 <- expr7[filt7, ]
# dim(expr7)
...
pheno1$Metastasis <- factor(pheno1$Metastasis, levels = c("No_Mets", "Mets"))
...
pheno2$Metastasis <- factor(pheno2$Metastasis, levels = c("No_Mets", "Mets"))
...
pheno4$Metastasis <- factor(pheno4$Metastasis, levels = c("No_Mets", "Mets"))
...
pheno3$Metastasis <- factor(pheno3$Metastasis, levels = c("No_Mets", "Mets"))
...
Pheno5$Metastasis <- factor(Pheno5$Metastasis, levels = c("No_Mets", "Mets"))
...
# 
# Pheno6$Metastasis <- factor(Pheno6$Metastasis, levels = c("No_Mets", "Mets"))
...
# Pheno7
# Processed
Pheno7$Metastasis <- factor(Pheno7$Metastasis, levels = c("No_Mets", "Mets")) 

#########
# Pheno8
#table(pheno8$Metastasis)
#levels(pheno8$Metastasis)

########
# Pheno9
#table(pheno9$Metastasis)
#levels(pheno9$Metastasis)
...
##############
## Save the natural history cohort
natHistory_expression <- as.data.frame(expr7)
natHistory_expression <- natHistory_expression %>%
  mutate(gene = rownames(.)) %>%
  relocate(gene)

write_csv(natHistory_expression, file = './data/natHistory_expression.csv')

select_columns <- c('celfile_name', 'Metastasis')
natHistory_label <- Pheno7[, select_columns]
write_csv(natHistory_label, file = './data/natHistory_label.csv')

#########################################################################

## Combine expression and phenotype

AllPheno <- list(pheno1, pheno2, pheno3, pheno4, Pheno5, Pheno7, pheno10)
names(AllPheno) <- c("GSE116918", "GSE55935", "GSE51066", "GSE46691", "GSE41408", "JHU", "GSE70769")

AllExpr <- list(expr1, expr2, expr3, expr4, expr5, expr7, expr10)
names(AllExpr) <- c("GSE116918", "GSE55935", "GSE51066", "GSE46691", "GSE41408", "JHU", "GSE70769")

GroupMets <- list(pheno1$Metastasis, pheno2$Metastasis, pheno3$Metastasis, pheno4$Metastasis, Pheno5$Metastasis, Pheno7$Metastasis, pheno10$Metastasis)
names(GroupMets) <- c("GSE116918", "GSE55935", "GSE51066", "GSE46691", "GSE41408", "JHU", "GSE70769")
...
############################
# GSE55935 Out
# Train <- c("GSE116918", "GSE51066","GSE46691", "GSE41408", "JHU", "GSE70769")
# Test <- c("GSE55935")
# 
# ## Training
# trainMat <- do.call("cbind", exprsMetastasis[Train])
...
############################
# GSE51066 Out
# Train <- c("GSE116918", "GSE55935","GSE46691", "GSE41408", "JHU", "GSE70769")
# Test <- c("GSE51066")
# 
# ## Training
# trainMat <- do.call("cbind", exprsMetastasis[Train])
...
############################
# GSE46691 Out
# Train <- c("GSE116918", "GSE55935","GSE51066", "GSE41408", "JHU", "GSE70769")
# Test <- c("GSE46691")
# 
# ## Training
# trainMat <- do.call("cbind", exprsMetastasis[Train])
...
############################
# GSE41408 Out
# Train <- c("GSE116918", "GSE55935","GSE51066", "GSE46691", "JHU", "GSE70769")
# Test <- c("GSE41408")
# 
# ## Training
# trainMat <- do.call("cbind", exprsMetastasis[Train])
...
Train <- c("GSE116918", "GSE55935","GSE51066", "GSE46691", "GSE41408", "GSE70769")
Test <- c("JHU")
...
# Save for cross-study validation
save(trainMat, trainGroup, testMat, testGroup, file = "./outs/MetastasisData_JHUOut.rda")
...
############################
# GSE70769 Out
# Train <- c("GSE116918", "GSE55935","GSE51066", "GSE46691", "GSE41408", "JHU")
# Test <- c("GSE70769")
# 
# ## Training
# trainMat <- do.call("cbind", exprsMetastasis[Train])
...
# ################################################################
# ## PSA
# AllPheno$GSE116918$PSA <- as.numeric(AllPheno$GSE116918$`psa (ng/ml):ch1`)
# AllPheno$GSE41408$PSA <- as.numeric(AllPheno$GSE41408$`initial psa (ug/l):ch1`)
# AllPheno$JHU$PSA <- as.numeric(AllPheno$JHU$preop_psa)
# AllPheno$GSE70769$PSA <- as.numeric(AllPheno$GSE70769$`psa at diag:ch1`)
# 
# 
# AllPSA <- lapply(AllPheno, function(x) {
#   i <- grep("^PSA", colnames(x))
#   if (length(i) == 0) out <- factor(rep("", nrow(x)))
#   else x <- factor(x[, i  ])
# })
# 
# AllPSA <- unlist(AllPSA)
# 
# AllPSA[AllPSA == ""] <- NA
# AllPSA <- as.numeric(as.character(AllPSA))
# #AllPSA <- round(as.numeric(AllPSA))
# 
# summary(AllPSA)
# 
# #########################################################
# ### Assemble in one data.frame and turn numeric
# covs <- data.frame(Stage = AllStage, Age = allAGE, Gleason = AllGleason, PSA = AllPSA)
# 
